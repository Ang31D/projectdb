...
class init(Script):
	...
	def _truncate_table(self, args):
		...
		if dry_mode:
			out_trunc_statement = "'%s'" % table_name
			if db_name is not None:
				out_trunc_statement = "'%s' from db('%s')" % (table_name, db_name)
			print("[*] truncate table %s" % out_trunc_statement)
			if not db.table_exists_in_db(db_conn, db_name, table_name):
				print("[!] %s; dry mode warning: table not found - '%s'" % (self.name, table_name))
			else:
				print("[!] %s; dry mode - would have truncated %s" % (self.name, out_trunc_statement))
			db.close(db_conn)
			return
