...
class init(Script):
	...
	def _on_help(self):
		...
		help_output += "\n    update='<column_name>'  - column to update on"
		help_output += "\n    value='<value>'         - value to update to"
		help_output += "\n"
	...
	def _on_run(self, args):
		...
		if 'update' not in args:
			print("[!] error: %s; missing required 'update' option" % self.name)
			return
		if 'value' not in args:
			print("[!] error: %s; missing required 'value' option" % self.name)
			return
		...
		if dry_mode:
			print("[!] dry-mode - no records updated")
			return
	...
	def _update_record(self, args):
		...
		query = self._build_update_query(args)
		...
		try:
			...
			return False
	def _build_update_query(self, args):
		table = args["table"]["value"]
		column = args["column"]["value"]
		match_on = args["match-on"]["value"]
		update_column = args["update"]["value"]
		new_value = args["value"]["value"]

		query = '''UPDATE %s SET %s = "%s" WHERE %s = "%s"
		''' % (table, update_column, new_value, column, match_on)
		return query
